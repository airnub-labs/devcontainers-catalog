...
import { generateStackTemplate } from "../stacks.js";
...
describe("generateStackTemplate", () => {
  ...
  it("merges browser services, reassigns ports, and preserves comments", async () => {
    ...
    expect(composeContent).toMatch(/\"3001:3000\"/);
    ...
    expect(composeDoc.services.browser.ports).toContain("3001:3000");
    ...
    expect(devcontainer.forwardPorts).toEqual(expect.arrayContaining([3000, 3001, 59000]));
    expect(devcontainer.portsAttributes["3001"].label).toBe("Browser UI");
